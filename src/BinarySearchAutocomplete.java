...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public BinarySearchAutocomplete(String[] terms, double[] weights) {
		if (terms == null || weights == null) throw new NullPointerException("One or more arguments null");
	...
	public static int firstIndexOf(Term[] a, Term key, Comparator<Term> comparator) {
		...
		if (a == null || key == null || comparator == null) throw new NullPointerException("One or more arguments null");
	...
	public Iterable<String> topMatches(String prefix, int k) {
		if (prefix == null) throw new NullPointerException("argument null");	
		...
		// pq now stores top k matches, with lowest-weight term as pq.peek()
		LinkedList<String> list = new LinkedList<>();// create new LinkedList to return
		while (pq.size() > 0) {
	...
	public String topMatch(String prefix) {
		...
		int minIndex = firstIndexOf(myTerms, new Term(prefix, 0), new Term.PrefixOrder(prefix.length()));
		int maxIndex = lastIndexOf(myTerms, new Term(prefix, 0), new Term.PrefixOrder(prefix.length()));
		if (minIndex == -1 || maxIndex == -1) return new String();
		String topMatch = "";
		double topWeight = 0;
		for(int i = minIndex; i <= maxIndex; i++){
			if (myTerms[i].getWeight() >= topWeight) {
				topWeight = myTerms[i].getWeight();
				topMatch = myTerms[i].getWord(); 
