...
public class Term implements Comparable<Term> {
	...
	public Term(String word, double weight) {
		// TODO: Complete Term constructor //should be completed
		myWord = word;
		myWeight = weight;
		if (word == null) throw new NullPointerException("word is null " + word);
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			// TODO: Implement compare //should be completed
			String vWord = v.getWord();
			String wWord = w.getWord();
			if (vWord.length() < this.r) 
				vWord = vWord.substring(0, vWord.length());
			else
				vWord = vWord.substring(0,this.r);
			if (wWord.length() < this.r) 
				wWord = wWord.substring(0, wWord.length());
			else
				wWord = wWord.substring(0,this.r);
	...
	public static class ReverseWeightOrder implements Comparator<Term> {
		public int compare(Term v, Term w) {
			// TODO: Implement ReverseWeightOrder.compare  //should be completed
			Double vWeight = v.myWeight;
			Double wWeight = w.myWeight;
			return wWeight.compareTo(vWeight);
		}
	}
	...
	public static class WeightOrder implements Comparator<Term> {
		public int compare(Term v, Term w) {
			// TODO: Implement WeightOrder.compare  //should be completed
			Double vWeight = v.myWeight;
			Double wWeight = w.myWeight;
			// Collections.sort(list, Comparator.comparing(Term::getWeight).reversed()); //still confused by this. see slides from Friday Nov 17
			return vWeight.compareTo(wWeight);
		}
	}
}
